...
# Sincroniza desde SOC_ORD_Project/05_Web_Promocional hacia SOC_ORD_WEB y publica en GitHub Pages.
# ------------------------------------------------------------------------------
...
rsync -av --delete --exclude='.git' --exclude='.gitignore' "$SCRIPT_DIR/" "$WEB_DIR/"

cd "$WEB_DIR"

git add -A
TIMESTAMP=$(date +"%Y-%m-%d %H:%M:%S")
git commit -m "Auto-deploy: $TIMESTAMP - VISORD_demo & SOC_ORD Web sync" || true
